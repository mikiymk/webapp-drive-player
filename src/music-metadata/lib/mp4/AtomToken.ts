...
import { UINT32_BE, StringType, UINT64_BE, UINT8, UINT24_BE, UINT16_BE, Uint8ArrayType, INT16_BE, INT32_BE, INT8, INT24_BE } from "token-types";
...
export class DataAtom implements IGetToken<IDataAtom> {
  ...
  public get(buf: Uint8Array, off: number): IDataAtom {
    return {
      ...
      value: Buffer.from(
        new Uint8ArrayType(this.len - 8).get(buf, off + 8)
      ),
    };
  }
}
...
export const SoundSampleDescriptionV0: IGetToken<ISoundSampleDescriptionV0> = {
  ...
  get(buf: Buffer, off: number): ISoundSampleDescriptionV0 {
    return {
      ...
      sampleRate:
        UINT16_BE.get(buf, off + 8) +
        UINT16_BE.get(buf, off + 10) / 10000,
    };
  },
};
